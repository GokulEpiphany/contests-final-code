...
def run():
    ...
    state_dict = torch.load('fastai-data/models/snake-detection-model.pth') #our trained model which predicts 4 coordinates,bbox 
    learn.model.load_state_dict(state_dict['model'])
    if not os.path.exists('preprocessed-images'):
        os.makedirs('preprocessed-images') #directory to store preprocessed test images
    ...
    src = (ImageList.from_folder(path='fastai-data').split_by_rand_pct(0.05).label_from_folder())
    ...
    learn.load('b5-seed-911-round-2') #single model providing 0.861, clean and no assertion error
    ...
    #around 7 predictions causes assertion error, for now submit them as class-204 - Not needed


    df_test.to_csv(predictions_output_path,index=False) #directly write to output path
